...
def water_proxy(df):
    """
    Rolling 2-week cumulative precipitation as a standing-water proxy.
    BUG FIX: was 'PROCTOTCORR_sum' (typo) → corrected to 'PRECTOTCORR_sum'.
    """
    df = df.sort_values(by=["district", "epi_week", "year"]).reset_index(drop=True)

    df["water_proxy"] = (
        df.groupby("district")["PRECTOTCORR_sum"]          # ← fixed typo
          .transform(lambda s: s.rolling(window=2, min_periods=1).sum())
    )
...
def HandleDataInconsistency(data):
    ...
    data = data.copy()                         # avoid mutating the caller's df
    ...
    return data                                # always return the cleaned copy
